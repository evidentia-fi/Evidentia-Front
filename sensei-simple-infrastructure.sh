...
# Simplified Sensei OP Stack Infrastructure Setup
# Uses smaller droplet sizes to work within account limits

set -e

# Configuration
DO_API_TOKEN="${DO_API_TOKEN:-your_digitalocean_token_here}"
REGION="nyc1"  # Use nyc1 which seems to have available resources
SSH_KEY_NAME="sensei-ssh-key"  # You'll need to create this SSH key first

# Simplified droplet configurations (smaller sizes)
# Format: name:size:vcpus:ram:disk
DROPLETS="
sequencer:s-2vcpu-4gb:2:4:80
batcher:s-1vcpu-2gb:1:2:50
proposer:s-1vcpu-2gb:1:2:50
replica-1:s-2vcpu-4gb:2:4:80
replica-2:s-2vcpu-4gb:2:4:80
explorer:s-1vcpu-2gb:1:2:50
monitoring:s-1vcpu-1gb:1:1:25
"
...
print_success "Simplified infrastructure setup completed!"
